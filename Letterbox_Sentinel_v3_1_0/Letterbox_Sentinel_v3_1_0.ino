/*
  ...
  - IR active Monday-Saturday, 07:30-17:00
  ...
  - Multiple deliveries per day, even while Mail Waiting remains ON
  - 30-second IR lockout after each genuine delivery, then automatic rearm
  - Mail Waiting remains persistent until NFC/manual collection
...
// 2. Enter the hostname or IP address of your MQTT broker.
// 3. Enter your MQTT username and password.
// 4. Choose whether to use DHCP or a fixed/static IP address.
// 5. Set your timezone, site altitude and mail-monitoring hours.
//
// NETWORK ADDRESSING
// ------------------
// DHCP is enabled by default because it works on almost every home network,
// regardless of whether the LAN uses 192.168.0.x, 192.168.1.x, 10.x.x.x,
// 172.16.x.x, or another private address range.
//
// For a dependable fixed address, the recommended approach is:
//   - Leave USE_STATIC_IP set to false here; and
//   - Create a DHCP reservation in your router for this device's MAC address.
//
// Only set USE_STATIC_IP to true when you understand your router's subnet,
// gateway, DNS and available address range.
...
// This may be either:
//   - A hostname, such as "homeassistant.local" or "mqtt.local"; or
//   - A numeric IP address, such as "192.168.1.50".
//
// IMPORTANT: Home Assistant is not always the MQTT broker. If you use the
// Mosquitto add-on, the broker normally runs on the Home Assistant machine,
// so the Home Assistant hostname/IP will usually work.
...
// OTA settings used for wireless Arduino uploads after the first USB flash.
const char* OTA_HOSTNAME = "letterbox-sentinel";
const char* OTA_PASSWORD = "CHANGE_OTA_PASSWORD";

// DHCP is the safe and portable default.
const bool USE_STATIC_IP = false;

// These values are ignored while USE_STATIC_IP is false.
// Edit all four values before enabling a static IP.
IPAddress DEVICE_IP(192, 168, 1, 220);
IPAddress GATEWAY_IP(192, 168, 1, 1);
IPAddress SUBNET_MASK(255, 255, 255, 0);
IPAddress DNS_SERVER(192, 168, 1, 1);

// Melbourne/Victoria timezone, including automatic daylight saving.
// Installations elsewhere must replace this POSIX timezone rule.
const char* TIMEZONE_RULE = "AEST-10AEDT,M10.1.0,M4.1.0/3";
const char* NTP_SERVER_1 = "pool.ntp.org";
const char* NTP_SERVER_2 = "time.google.com";

// Site elevation used to convert raw station pressure to sea-level pressure.
// Change this to the approximate metres above sea level at your installation.
const float SITE_ALTITUDE_METERS = 230.0F;

// Battery calibration for the documented 330k/100k divider and this D1 Mini.
// Compare the Home Assistant voltage with a multimeter and adjust
// BATTERY_CALIBRATION if required.
const float ADC_FULL_SCALE_V = 3.20F;
const float BATTERY_DIVIDER_RATIO = 5.35F;
const float BATTERY_CALIBRATION = 0.975F;

// Daily IR monitoring window.
// Current default: Monday-Saturday, 07:30 until 17:00; Sunday disabled.
const uint8_t IR_START_HOUR = 7;
const uint8_t IR_START_MINUTE = 30;
const uint8_t IR_END_HOUR = 17;
const uint8_t IR_END_MINUTE = 0;

// Time the IR emitter remains off after a genuine delivery.
// This prevents one item of mail from being counted more than once, while
// allowing another later delivery to be detected even if mail is still waiting.
const unsigned long IR_POST_DELIVERY_LOCKOUT_MS = 30000;
...
// During Monday-Saturday 07:30-17:00 the ESP remains awake.
// Before the first real letter it continuously monitors the IR beam.
// Each genuine delivery starts a short IR lockout to prevent duplicate counts.
// Mail Waiting remains ON until NFC/manual collection, but the detector rearms
// automatically after the lockout so later deliveries are still counted.
//
...
unsigned long irLockoutUntilMs = 0;
...
bool irDeliveryLockoutActive() {
  if (irLockoutUntilMs == 0) return false;

  // Signed subtraction makes millis() rollover-safe.
  return static_cast<long>(irLockoutUntilMs - millis()) > 0;
}
...
void recordMailDetection(const char* source) {
  ...
  JsonDocument event;
  event["source"] = source;
  event["letter_count"] = persistent.letterCount;
  event["timestamp"] = epochToIso8601(persistent.lastMailEpoch);
  event["battery_voltage"] = readBatteryVoltage();

  char payload[384];
  serializeJson(event, payload, sizeof(payload));
  mqttClient.publish(TOPIC_MAIL_EVENT, payload, false);

  Serial.print("MAIL DETECTED. Source: ");
  Serial.println(source);

  if (strcmp(source, "ir") == 0) {
    irLockoutUntilMs = millis() + IR_POST_DELIVERY_LOCKOUT_MS;
    setEmitterActive(false);
    Serial.println("IR delivery lockout active for 30 seconds");
  }
}
...
void clearMailWaiting(const char* collector) {
  ...
  if (collector && strlen(collector) > 0) {
    strlcpy(
      persistent.lastCollector,
      collector,
      sizeof(persistent.lastCollector)
  ...
  JsonDocument event;
  event["collector"] = persistent.lastCollector;
  event["timestamp"] = epochToIso8601(persistent.lastCollectionEpoch);
  event["letter_count"] = persistent.letterCount;

  char payload[384];
  serializeJson(event, payload, sizeof(payload));
  mqttClient.publish(TOPIC_COLLECTION_EVENT, payload, false);

  Serial.print("Mail cleared by: ");
  Serial.println(persistent.lastCollector);
}
...
bool insideIRClockWindow() {
  // This function is deliberately CLOCK-ONLY. Mail-waiting IR shutdown must
...
void connectWiFi() {
  ...
  // DHCP is used by default. Static addressing is applied only when the
  // installer explicitly enables it in the USER CONFIGURATION section.
  if (USE_STATIC_IP) {
    const bool staticConfigAccepted = WiFi.config(
      DEVICE_IP,
      GATEWAY_IP,
      SUBNET_MASK,
      DNS_SERVER
    ...
    Serial.println(
      staticConfigAccepted
        ? "Static IP configuration accepted"
        : "WARNING: Static IP configuration failed; attempting connection"
    );
  } else {
  ...
  if (
    ...
    Serial.println(USE_STATIC_IP ? "Static configuration" : "DHCP");
  } else {
    Serial.println(
...
void loop() {
  ...
  // Monday-Saturday 07:30-17:00: remain awake and monitor for every delivery.
  // After each genuine delivery the emitter pauses for 30 seconds, then rearms.
